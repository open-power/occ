 ...
 * Function Specifications:
 *
 * Name: gpe_get_quad_data
 *
 * Description:  get quad sensor data from all quads specified by a quad mask
 *
 * Inputs:       cmd is a pointer to IPC msg's cmd and cmd_data struct
 *
 * Outputs:      error: sets rc, address, and ffdc in the cmd_data's
...
void gpe_get_quad_data(ipc_msg_t* cmd, void* arg)
{
    uint32_t rc;    // return code
    ipc_async_cmd_t *async_cmd  = (ipc_async_cmd_t*)cmd;
    ipc_quad_data_parms_t *args = (ipc_quad_data_parms_t*) async_cmd->cmd_data;

    rc = get_quad_data(args->quad_mask,     // quad mask
                       args->data);         // QuadData *

    if(rc)
    {
        PK_TRACE("gpe_get_quad_data: get_quad_data failed, rc = 0x%08x, quad_mask = 0x%08x",
                 rc, args->quad_mask);

    }
    gpe_set_ffdc(&(args->error), args->quad_mask,
                 GPE_RC_GET_QUAD_DATA_FAILED, rc);

    // send back a response, IPC success even if ffdc/rc are non zeros
    rc = ipc_send_rsp(cmd, IPC_RC_SUCCESS);
    if(rc)
    {
        PK_TRACE("gpe_get_quad_data: Failed to send response back. Halting GPE0", rc);
        gpe_set_ffdc(&(args->error), 0x00, GPE_RC_IPC_SEND_FAILED, rc);
        pk_halt();
    }
