...
typedef enum
{
    DBUG_DUMP_WOF_DATA        = 0x01,  // dynamic WOF data
...
//  free  = 0x21,
    ...
    DBUG_GET_AME_SENSOR_AVG   = 0xA7,
    DBUG_GET_AND_CLEAR_AME_SENSOR_AVG = 0xAC  // does DBUG_GET_AME_SENSOR_AVG and clears sensor data
} DBUG_CMD;
...
// Max number of sensors that can be returned with DBUG_GET_AME_SENSOR_AVG command (subtract 2 bytes for num_sensors)
#define CMDH_DBUG_AVG_MAX_NUM_SENSORS  ((CMDH_FSP_RSP_DATA_SIZE-2) / sizeof(cmdh_dbug_sensor_list_avg_t))
...
// Used by OCC firmware to respond DBUG_GET_AME_SENSOR_AVG and DBUG_GET_AND_CLEAR_AME_SENSOR_AVG commands
typedef struct __attribute__ ((packed))
{
    struct                  cmdh_fsp_rsp_header;
    uint16_t                num_sensors;
    cmdh_dbug_sensor_list_avg_t sensor[CMDH_DBUG_AVG_MAX_NUM_SENSORS];
}cmdh_dbug_get_sensor_avg_resp_t;
