...
void dimm_reset_master(ipc_msg_t* cmd, void* arg)
{
    ...
    if(rc)
    {
        PK_TRACE("E>dimm_reset_master: I2C_IMM_RESET_I2C putscom 0x%08X->0x%08X%08X FAILED. rc = 0x%08x",
    ...
    else
    {
        ...
        if(rc)
        {
            PK_TRACE("E>dimm_reset_master: I2C_BUSY_REGISTER putscom 0x%08X->0x%08X%08X FAILED. rc = 0x%08x",
                     scomAddr, WORD_HIGH(regValue), WORD_LOW(regValue), rc);
            gpe_set_ffdc(&(args->error), scomAddr, GPE_RC_SCOM_PUT_FAILED, rc);
        }
        else
...
void dimm_reset_slave(ipc_msg_t* cmd, void* arg)
{
    ...
    if(rc)
    {
        PK_TRACE("E>dimm_reset_slave: I2C_MODE_REG putscom 0x%08X->0x%08X%08X FAILED. rc = 0x%08x",
                 scomAddr, WORD_HIGH(regValue), WORD_LOW(regValue), rc);
        gpe_set_ffdc(&(args->error), scomAddr, GPE_RC_SCOM_PUT_FAILED, rc);
    }
    else
    {
        ...
        if(rc)
        {
            PK_TRACE("E>dimm_reset_slave: I2C_COMMAND_REG putscom 0x%08X->0x%08X%08X FAILED. rc = 0x%08x",
                     scomAddr, WORD_HIGH(regValue), WORD_LOW(regValue), rc);
            gpe_set_ffdc(&(args->error), scomAddr, GPE_RC_SCOM_PUT_FAILED, rc);
        }
        else
...
void dimm_reset_slave_status(ipc_msg_t* cmd, void* arg)
{
    ...
    if(rc)
    {
        PK_TRACE("E>dimm_reset_slave_status: I2C_STATUS_REG getscom 0x%08X FAILED. rc = 0x%08x - STATUS #1",
                 scomAddr, rc);
    ...
    else
    {
        ...
        if (regValue & STATUS_COMPLETE_MASK)
        {
            ...
            PK_TRACE("E>dimm_reset_slave_status: I2C Slave Port %d Reset completed", args->i2cPort);

            if ((regValue & STATUS_ERROR_OR_COMPLETE_MASK) != STATUS_COMPLETE_MASK)
            {
                // I2C errors found
                PK_TRACE("E>dimm_reset_slave_status: I2C errors in status register: 0x%08X%08X",
