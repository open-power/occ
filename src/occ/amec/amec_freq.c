...
void amec_slv_voting_box(void)
{
    ...
    for (k=0; k<MAX_NUM_CORES; k++)
    {
        if(CORE_PRESENT(k))
        {
 	    ...
 	    // Check frequency request generated by WOF algorithm // @cl020
 	    if(g_amec_wof_f_vote < l_core_freq)
 	    {
 		l_core_freq = g_amec_wof_f_vote;
 		l_core_reason = AMEC_VOTING_REASON_WOF;
 	    }
